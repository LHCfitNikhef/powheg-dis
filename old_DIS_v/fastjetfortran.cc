...
void fastjetppgenkt_(const double * p, const int & npart,                   
  ...
  void fastjetppgenktexcl_(const double * p, const int & npart,                   
			   const double & R, const double & palg, const int & nexcl,
			   double * f77jets, int & njets, int * f77jetvec) {
    ...
    fj::ClusterSequence cs(input_particles, jet_def);
    // extract jets (pt-ordered)
    vector<fj::PseudoJet> jets = sorted_by_pt(cs.exclusive_jets(nexcl));
    njets = jets.size();
