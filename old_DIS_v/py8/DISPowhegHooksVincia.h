...
public:
  ...
  bool initAfterBeams() {
    ...
    dis_map     = settingsPtr->mode("POWHEG:dis_map");
  ...
  inline double pTpowheg(const Event &e, int i, int j, bool FSR) {
	...
	( e[iInA].e()  + e[iInB].e()  );
      ...
      Vec4 pin, pout;
      double stilde, csi, omy, Ei, Ej;
      int iupperisr = dis_map;
	...
	pow2(iVecBst.e() + jVecBst.e()) );*/
      ...
      pin = e[iInA].p()+e[iInB].p(); 

      //Post-branching total momentum, obtained by summing over the final-state partons
	...
	if (e[ii].isFinal()){
      ...
      Ej     = (e[j].p()*pout)/sqrt(pout.m2Calc());
      Ei     = (e[i].p()*pout)/sqrt(pout.m2Calc());
      csi    = 2. * e[j].p()*pout/(pout.m2Calc());
      //For final-state g2gg and g2qqbar take as csi always the softer fraction
      if(FSR && ( (e[j].id()==21 && e[j].id()==21) || (e[j].id()+e[i].id()==0))){
	csi = min(csi, 2. * e[i].p()*pout/(pout.m2Calc()));
      }

      //1-cthTh, with Th between the emitter and the spectator in the event frame
      //NB: if i is in the initial state, e[i] and Ei are not correct, but their
      //    ratio is, and here we use e[i]/Ei
      omy    = (e[i].p()*e[j].p())/Ei/Ej;

      // scalup **2  = csi^2 sTilde (1-y)/2            FSR
      // scalup **2  = csi^2 sTilde (1-y)/(2-csi(1+y)) ISR
      
      pTnow = sqrt(stilde*omy)*csi;
      if(FSR){
	pTnow = pTnow / sqrt(2.);
      }else{
	if (iupperisr == 1){
	  pTnow = pTnow/sqrt(2.-csi*(2.-omy));
	} else
	  {
	    pTnow = pTnow * sqrt((2.-omy)/(4.*(1.-csi*(1.-omy)*(1.-omy))));
	  }
      }
