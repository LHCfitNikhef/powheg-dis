...
!  pwhgfill  :  fills the histograms with data
     ...
     $     '___30-50','___50-70','__70-100', '_100-200'/) 
      ...
      call bookupeqbins('brd_zE',0.5d0/100d0,0d0,0.5d0)
      call bookupeqbins('brd_zE_log',5d0/100d0,-5d0,0d0)
      do i = 2,n
         ...
         call bookupeqbins('tauP'//trim(Qchar(i)),0.1d0,0d0,1d0)
         ! call bookupeqbins('tauP'//trim(Qchar(i)),0.01d0,0d0,1d0)
         call bookupeqbins("tau_CP"//trim(Qchar(i)),0.01d0,0d0,0.5d0)      
         ! call bookupeqbins("brd_zEP"//trim(Qchar(i)),0.01d0,0d0,0.5d0)
         call bookupeqbins("brd_zEP"//trim(Qchar(i)),0.05d0,0d0,0.5d0)
         call bookupeqbins("rhoP"//trim(Qchar(i)),0.03125d0,0d0,0.25d0)
         call bookupeqbins("cprEP"//trim(Qchar(i)),0.1d0,0d0,1d0)

         call bookupeqbins('tau_zQ'//trim(Qchar(i)),0.05d0,0d0,0.5d0) ! Not in the H1 analysis, but easy to get at NLL
      end do
...
!     we need to tell to this analysis file which program is running it
      ...
      real * 8 qevsh(4,0:4), kt2, norm, Ecur, pcur, ELong, cosTheta
      external obsf77
      real *8, parameter ::  Qbins(1:n) =
     $     (/14d0, 16d0, 20d0, 30d0, 50d0, 70d0, 100d0, 200d0/)
      character(len=8), parameter ::  Qchar(1:n) =
     ...
     $     '___30-50','___50-70','__70-100', '_100-200'/) 
      ...
      interface
         subroutine evs_tauzE_DIS(pcur, var)
         integer, parameter  :: dp = selected_real_kind(15)
         real(dp), intent(in)  :: pcur(:,:)
         real(dp), intent(out) :: var
         end       
      ...
      interface
         subroutine evs_tautE_DIS(pcur, var)
         integer, parameter  :: dp = selected_real_kind(15)
         real(dp), intent(in)  :: pcur(:,:)
         real(dp), intent(out) :: var
         end       
      ...
      interface 
         subroutine evs_brdzE_DIS(pcur, var)
         integer, parameter  :: dp = selected_real_kind(15)
         real(dp), intent(in)  :: pcur(:,:)
         real(dp), intent(out) :: var
         end       
      ...
      interface 
         subroutine evs_CprE_DIS(pcur, var)
         integer, parameter  :: dp = selected_real_kind(15)
         real(dp), intent(in)  :: pcur(:,:)
         real(dp), intent(out) :: var
         end       
      ...
      interface 
         subroutine evs_rhoE_DIS(pcur, var)
         integer, parameter  :: dp = selected_real_kind(15)
         real(dp), intent(in)  :: pcur(:,:)
         real(dp), intent(out) :: var
         end       
      ...
      ! if(WHCPRG.eq.'LHE   ' .and. rad_kinreg/=3) return 
      nlep = 0
      npartons = 0
      pquark = 0d0
      plep = 0d0
...
!     Final states
               ...
               cosTheta = -phep(3,i) / NORM2(phep(1:3,i))
               if(abs(idhep(i)).eq.11) then
                  if (DACOS(cosTheta) < 4d0*pi/180d0 .or. DACOS(cosTheta) > 154d0*pi/180d0) cycle
                  nlep = nlep + 1
                  plep(1:4,nlep) = phep(1:4,i)
                  ELong = ELong + phep(4,i) * (1d0 - cosTheta)
                  ! For now everything not an electron is assumed a
                  ! parton. If we run with QED shower we have to be
                  ! careful
               else 
                  if (DACOS(cosTheta) < 4d0*pi/180d0 .or. DACOS(cosTheta) > 177d0*pi/180d0) cycle
                  npartons = npartons + 1
      ...
      if (ELong  < 40d0 .or. ELong > 70d0) then
         return
      endif
...
!     -- Thrust with respect to the photon axis (normalized to E in current hemishere)
      call evs_tauzE_DIS(pevsh(:,:npartons),tau)
...
!     -- Thrust with respect to the photon axis (normalized to Q/2)
      ...
      do i=2,8 
         if (Qbins(i-1)< sqrt(Q2) .and. sqrt(Q2)< Qbins(i)) then
            ...
            ! Change the normalisation from E to |p|
            call filld('tauP'//trim(Qchar(i)),tau*Ecur/pcur,dsig)
            call filld('tau_CP'//trim(Qchar(i)),tau_C*Ecur/pcur,dsig)
            call filld('brd_zEP'//trim(Qchar(i)),brd_Ze*Ecur/pcur,dsig) 
            call filld('rhoP'//trim(Qchar(i)),rho*Ecur/pcur,dsig)
            call filld('cprEP'//trim(Qchar(i)),cprE*Ecur/pcur,dsig)
            
            call filld('tau_zQ'//trim(Qchar(i)),tau_q,dsig)
         endif
      enddo
