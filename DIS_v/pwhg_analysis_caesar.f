...
!  pwhgfill  :  fills the histograms with data
     ...
     $     '___30-50','___50-70','__70-100', '_100-200'/) 
      ...
      call bookupeqbins('tau_zQ',0.01d0,0d0,1d0)
      call bookupeqbins("brd_zE",0.01d0,0d0,0.5d0)
      
      call bookupeqbins('logtau_zQ',0.15d0,-15d0,0d0)
      call bookupeqbins("logbrd_zE",0.15d0,-15d0,0d0)
...
!     we need to tell to this analysis file which program is running it
      ...
      real * 8 qevsh(4,0:4), kt2, norm, Ecur, pcur
      real *8, parameter ::  Qbins(1:n) =
     $     (/14d0, 16d0, 20d0, 30d0, 50d0, 70d0, 100d0, 200d0/)
      character(len=8), parameter ::  Qchar(1:n) =
     $     (/'____0-14','___14-16','___16-20','___20-30',
...
!     -- Broadening with respect to the photon axis (normalized to E)       
      call evs_brdzE_DIS(pevsh(:,1:npartons),brd_zE)
...
!     -- Thrust with respect to the photon axis (normalized to Q/2)
      call evs_tauzQ_DIS(pevsh(:,1:npartons),qevsh(:,0),tau_q)
      
      if(brd_zE .le. 0d0) return
      if(tau_q .le. 0d0) return


      call filld('tau_zQ',tau_q,dsig)
      call filld('brd_zE',brd_Ze,dsig) 

      call filld('logtau_zQ',log(max(tau_q,1d-50)),dsig)
      call filld('logbrd_zE',log(max(brd_Ze,1d-50)),dsig) 
