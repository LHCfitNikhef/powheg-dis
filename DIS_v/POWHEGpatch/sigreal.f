      ...
      include 'pwhg_flg.h'
      include 'pwhg_par.h'
      real * 8 xrad(3),resreal(maxprocborn),www,sub
   ...
   !   #                        jac_over_csi_m,jac_over_csi_soft)
      ...
      ! FSR
   !    kn_emitter = 4
   !    call gen_real_phsp_fsr(xrad,jac_over_csi, jac_over_csi_coll,
   !   #                        jac_over_csi_soft)
      ! resreal(1) = kn_jacborn*jac_over_csi*kn_csitilde*kn_csimax**2/kn_sBeams

      ! return
      ! Phase space test end
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      do j=1,flst_nborn
         resreal(j)=0
      enddo
     ...
     2                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
                  ...
                  if(flg_nlotest) then
                     if(flg_detailedNLO) then
                        flst_currentalr = j
                        flst_currentuborn = flst_alr2born(j)
                        if(rrr0*www.ne.0d0) call analysis_driver(www*rrr0,1)
                        sub=-rrrc-rrr0s+rrrcs+remnant
                        if(sub*www.ne.0d0) call analysis_driver(www*sub,0)
                     endif
                     out1=out1+rrr0
                     if(flg_withsubtr) then
     ...
     #                  (25/kn_sbeams+(1-kn_y**2)*kn_csi)
                  ...
                  if(flg_nlotest) then
                     if(flg_detailedNLO) then
                        ...
                        flst_currentuborn = flst_alr2born(j)
                        if(rrr0*www.ne.0d0) call analysis_driver(www*rrr0,1)
                        sub = -rrr0s-rrrp-rrrm+rrrps+rrrms+remnant
                        if(sub*www.ne.0d0) call analysis_driver(www*sub,0)
                     endif
                     out1=out1+rrr0
                     if(flg_withsubtr) then
...
c look for equivalent alr's, write them out, and mark them for no checking             
     ...
     $                 .ne.r0s(alrp,jexp)) isequal=.false.
               enddo
               if(isequal) then
                     write(iun,'(2a,i3,a,i2,a,20(1x,i3))') label,' alr',alrp,' emitter ',
     1                    flst_emitter(alrp),', process ',(flst_alr(j,alrp),j=1,nlegreal)
                     write(iun,'(a,i3,a,i2,a,20(1x,i3))') ' == alr ',alr,' emitter ',
     ...
     1              .or. r0s(alr,jexp) /= r0s(alr,jexp)) then
      ...
      real * 8 jac_over_csi,
     1 jac_over_csi_coll,jac_over_csi_soft,r0(maxalr,nexp),
     2 r0c(maxalr,nexp),jac_over_csi_p,jac_over_csi_m,
...
c     exclude 2 particle production (l=4)!
            if(l > 4 .and. k < l) then
...
c no bornzerodamp with massive emitter
               ...
               if(dampfac == 0) then
                  if(kn_emitter.gt.2) then
                     r=r0(alr)*(1-kn_y)*(kn_csitilde * kn_csimax)**2
                  else
                     r=r0(alr)*(1-kn_y**2)*(kn_csitilde * kn_csimax)**2
     ...
     1                 dampfac)
               ...
               elseif(imode.eq.1) then
                  dampandcuts(alr) =dampandcuts(alr) * (1d0-dampfac)
               else
