...
c...writes initialization information to a les houches events file on unit nlf. 
      ...
      ! retrieve the energies from the powheg.input, as when running in fixed target mode they can differ from the true energies
...
!     write(buffer,110) idbmup(1),idbmup(2),ebmup(1),ebmup(2),
      write(buffer,110) idbmup(1),idbmup(2),ebeam1, ebeam2,
...
c     save last random number
 ...
 111  format(a,2(1x,i2),5(1x,e14.8))
     ...
     $        rwl_index,rwl_weight,rwl_seed,rwl_n1,rwl_n2,rad_kinreg,rad_ubfailrad
