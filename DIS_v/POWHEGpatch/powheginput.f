      subroutine set_powheginput_keyword(key,value)
      implicit none
      include 'pwhg_pwin.h'
      character * (*) key
...
c     Make sure that the keywork is present in the pwin_keyword array: 
      oldvalue = powheginput('#'//key)

      write(*,*) 'set_powhegiput_keyword: changing the value of '//
     1'keyword "'//trim(key)//'" from ',oldvalue,' to ',value 
      
      do j=1,pwin_numvalues
         if(key.eq.pwin_keywords(j)) then
            pwin_values(j) = value
            return
         endif
      enddo
