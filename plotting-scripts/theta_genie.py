...
file_names_lo = [
                '../DATA/theta/FASER/LO/dyn_scales.dat',
                '../DATA/theta/FASER/LO/dyn_scales-1.dat',
                 ]

files_lo_min = [
                '../DATA/theta/FASER/LO/dyn_scales_min.dat',
                '../DATA/theta/FASER/LO/dyn_scales_min-1.dat',
               ]

files_lo_max = [
                '../DATA/theta/FASER/LO/dyn_scales_max.dat',
                '../DATA/theta/FASER/LO/dyn_scales_max-1.dat',
               ]

file_names_nlo = [
                '../DATA/theta/FASER/NLO/dyn_scales.dat',
                '../DATA/theta/FASER/NLO/dyn_scales-1.dat',   
                  ]
 
file_names_min = [
                '../DATA/theta/FASER/NLO/dyn_scales_min.dat', 
                '../DATA/theta/FASER/NLO/dyn_scales_min-1.dat',
                  ]

file_names_max = [
                '../DATA/theta/FASER/NLO/dyn_scales_max.dat',
                '../DATA/theta/FASER/NLO/dyn_scales_max-1.dat',

genie_data = '../DATA/theta/GENIE/genie_nu_e.dat'
...
x_values_gen, y_errors_gen, sigma_gen = [], [], []
...
with open(genie_data, 'r') as file:
    for line in file:
        ...
        if len(columns) >= 2:
            ...
            sigma_gen.append(float(columns[0]))
...
genie_to_powheg_ratio = sigma_gen / y_values_nlo 
...
# GENIE
p1, = axs[0].plot(x_values_gen, sigma_gen, drawstyle="steps-post",
...
axs[0].set_xlim(0.005, 25E-3)
axs[0].set_ylim(50, 270)    # faser 
axs[0].set_xticklabels([])
axs[0].set_ylabel(r'$d\sigma / d\theta_l \ \mathrm{[pb/rad]}$')
axs[0].grid(linestyle='-', color='grey', linewidth=0.25)
axs[0].set_title(r'$E_\nu = \rm 1\ TeV$')

axs[1].set_xlim(0.005, 25E-3)
axs[1].set_ylim(0.85, 1.15)     # faser
axs[1].set_xlabel(r'$\theta_l \ \mathrm{[rad]}$')
axs[1].set_ylabel(r'$\rm Ratio \ to \ PW \ LO$', fontsize=8, labelpad=8)
...
#adjust layout
fig.subplots_adjust(top=0.9, left=0.15, bottom=0.13, right=0.95, hspace=0.12)

# Show plot
#plt.show()

plt.savefig('theta_44.pdf', format='pdf')
