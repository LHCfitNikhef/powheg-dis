...
file_names_lo = [
                '../DATA/theta/FASER/LO/fixed_scales.dat',
                '../DATA/theta/FASER/LO/fixed_scales-1.dat',
                 ]

files_lo_min = [
                '../DATA/theta/FASER/LO/fixed_scales_min.dat',
                '../DATA/theta/FASER/LO/fixed_scales_min-1.dat',
               ]

files_lo_max = [
                '../DATA/theta/FASER/LO/fixed_scales_max.dat',
                '../DATA/theta/FASER/LO/fixed_scales_max-1.dat',
               ]

file_names_nlo = [
                '../DATA/theta/FASER/NLO/fixed_scales.dat', 
                '../DATA/theta/FASER/NLO/fixed_scales-1.dat',
                
                  ]
 
file_names_min = [
                '../DATA/theta/FASER/NLO/fixed_scales_min.dat',
                '../DATA/theta/FASER/NLO/fixed_scales_min-1.dat',                         
                  ]

file_names_max = [
                '../DATA/theta/FASER/NLO/fixed_scales_max.dat', 
                '../DATA/theta/FASER/NLO/fixed_scales_max-1.dat',                     
                  ] 

pythia_file = '../DATA/theta/Pythia/faser.dat'
...
def pythia_data(file_name):
    ...
    with open(file_name, 'r') as file:
        for line in file:
            ...
            columns = line.split()
            if len(columns) == 2:
                theta_pyth.append(float(columns[0]))
                pyth_sigma.append((float(columns[1])) * sigtot / nevents / binwidth)
    return np.array(theta_pyth), np.array(pyth_sigma)
            
theta_pyth, pyth_sigma = pythia_data(pythia_file)
...
p1, = axs[0].plot(theta_pyth, pyth_sigma, drawstyle='steps-post',
...
axs[0].set_xlim(0.00375, 25E-3)
axs[0].set_ylim(40, 290)    # faser 
axs[0].set_xticklabels([])
axs[0].set_ylabel(r'$d\sigma / d\theta_l \ \mathrm{[pb/rad]}$')
axs[0].grid(linestyle='-', color='grey', linewidth=0.25)
axs[0].set_title(r'$E_\nu = \rm 1\ TeV$')



axs[1].set_xlim(0.00375, 25E-3)
axs[1].set_ylim(0.85,1.15)
axs[1].set_xlabel(r'$\theta_l \ \mathrm{[rad]}$')
axs[1].set_ylabel(r'$\rm Ratio \ to \ PW \ LO$', fontsize=8, labelpad=9)
...
#adjust layout
fig.subplots_adjust(top=0.9, left=0.16, bottom=0.12, right=0.95, hspace=0.12)

# Show plot
#plt.show()

plt.savefig('theta_42_faser.pdf', format='pdf')
