...
file_names_lo = [
                '../DATA/Energy-lep/FASER/LO/fixed_scales.dat',
                '../DATA/Energy-lep/FASER/LO/fixed_scales-1.dat',
                
                 ]

files_lo_min = [
                '../DATA/Energy-lep/FASER/LO/fixed_scales_min.dat',
                '../DATA/Energy-lep/FASER/LO/fixed_scales_min-1.dat',
               ]

files_lo_max = [
                '../DATA/Energy-lep/FASER/LO/fixed_scales_max.dat',
                '../DATA/Energy-lep/FASER/LO/fixed_scales_max-1.dat',
               ]

file_names_nlo = [
                '../DATA/Energy-lep/FASER/NLO/fixed_scales.dat',
                '../DATA/Energy-lep/FASER/NLO/fixed_scales-1.dat',
                  ]
 
file_names_min = [
                '../DATA/Energy-lep/FASER/NLO/fixed_scales_min.dat',
                '../DATA/Energy-lep/FASER/NLO/fixed_scales_min-1.dat',
                  ]

file_names_max = [
                '../DATA/Energy-lep/FASER/NLO/fixed_scales_max.dat',
                '../DATA/Energy-lep/FASER/NLO/fixed_scales_max-1.dat',
                  ] 

pythia_file = '../DATA/Energy-lep/Pythia/faser.dat'
...
def pythia_data(file_name):
    ...
    with open(file_name, 'r') as file:
        for line in file:
            ...
            columns = line.split()
            if len(columns) == 2:
                Elep_pyth.append(float(columns[0]))
                pyth_sigma.append((float(columns[1])) * sigtot / nevents / binwidth)
    return np.array(Elep_pyth), np.array(pyth_sigma)
            
Elep_pyth, pyth_sigma = pythia_data(pythia_file)
...
p1, = axs[0].plot(Elep_pyth, pyth_sigma, drawstyle='steps-post',
...
axs[0].set_xlim(100, 820)
#axs[0].set_ylim(0, 0.007)
axs[0].set_ylim(0.0005,0.0065)
#axs[0].set_yticks(np.arange(0.004, 0.005, 0.0005))      # neutrinos

axs[0].set_xticklabels([])
axs[0].set_ylabel(r'$d\sigma / dE_l \ \mathrm{[pb/GeV]}$')
...
#adjust layout
fig.subplots_adjust(top=0.9, left=0.19, bottom=0.12, right=0.95, hspace=0.12)

# Show plot
#plt.show()

plt.savefig('Elep_42_faser.pdf', format='pdf')
