...
c     check parameters
     ...
     $        //vetoname
      ...
      filenamedd="POWHEG+PYTHIA8-output-"//TRIM(rnd_cwhichseed)//TRIM(ADJUSTL(dists(1)%str))
      CALL dists(1)%write_to(filenamedd)
      filenamedd="POWHEG+PYTHIA8-output-"//TRIM(rnd_cwhichseed)//TRIM(ADJUSTL(dists(2)%str))
      CALL dists(2)%write_to(filenamedd)
      end
