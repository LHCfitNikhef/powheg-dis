...
c id is either btilde or remn
      ...
      integer, save :: nnnb = 0
      real * 8  t,csi,y,azi,beams(0:3,2),masses(nlegborn),xb1,xb2,x1,x2,jacreal,branchfactor
...
c decays are all stored by the handle radiation routine
      if(id == 'btilde') then
         call handle_radiations('inquire',nrad,0d0,0d0,0d0,0d0,0d0)
      elseif(id == 'remn') then
...
c     
            ...
            rad_kinreg = 0
            do j=1,nrad
               call handle_radiations('next',alr,t,csi,y,azi,branchfactor)
               if(t.lt.0) then
...
c     the current real becomes the new Born.
               ...
               if(em.le.2) then
                  if (flg_dis_isr) then
                     call gen_real_phsp_isr_rad_dis(nlegb,csi,y,azi,beams,xb1,xb2,pborn(:,1:nlegb),cmpborn(:,1:nlegb),x1,x2,preal(:,1:nlegr),cmpreal(:,1:nlegr),jacreal, branchfactor)
                  else
     ...
     3                 cmpborn(:,1:nlegb),x1,x2,preal(:,1:nlegr),cmpreal(:,1:nlegr),jacreal)
                  ...
                  if (flg_dis_fsr) then
                     call gen_real_phsp_fsr_rad_dis(nlegb,reslistborn,em
     +                ,csi,y,azi,xb1,xb2,masses,pborn(:,1:nlegb),cmpborn(:,1:nlegb),beams,x1,x2,preal(:,1:nlegr),cmpreal(:,1:nlegr),jacreal)
                  else
