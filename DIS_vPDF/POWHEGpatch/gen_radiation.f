...
c this call set to zero the number of stored radiations
            call handle_radiations('reset',0,0d0,0d0,0d0,0d0,0d0)
         endif
...
c generated.
               call handle_radiations('save',rad_alrsubp,t,kn_csi,kn_y,kn_azi,kn_branchfactor)
            endif
         endif
      enddo
      end
      ...
      if(rad_kinreg.eq.1) then
         ...
         !! For DIS, we use an approximation of kn_sreal = kn_sborn/(1-*kn_csi*kn_y**2)
         if(flg_dis_isr) then
            if (rad_iupperisr == 1) then
               pwhg_pt2=kn_sborn*(1d0+kn_y)*kn_csi**2/(2d0-kn_csi*(1d0-kn_y))
            else
               pwhg_pt2=(kn_sborn/4d0)*(1d0-kn_y**2)*kn_csi**2/(1d0-kn_csi*kn_y**2)
            endif
         endif
      else
         ...
         if(kn_masses(em).eq.0) then
            if(flst_bornrescurr(em).ne.0) then
               ...
               q2=kn_sreal
!! For DIS, we use an approximation of kn_sreal = kn_sborn
               if(flg_dis_fsr) q2 = kn_sborn
            endif
      ...
      if(rad_kinreg.eq.1) then
         if (flg_dis_isr) then
            if(rad_iupperisr.eq.1) then
               pwhg_upperb_rad = 1d0/(csi*(1+y))
            else if (rad_iupperisr.eq.2) then
               pwhg_upperb_rad = 1/((1-x)*(1-y**2))
            else
               write(*,*) ' rad_iupper=',rad_iupperisr,
     1              'has to be either 1 (asymmetric in y) or 2 (symmetric in y)'
                  call pwhg_exit(1)
            endif
         else
            if (em_rad_on) then
...
c     for now use the same
     ...
     1                 'alternative not implemented'
               ...
               if(flg_dis_fsr .and. rad_iupperfsr .ne. 1) then
                  write(*,*) ' rad_iupper=',rad_iupperfsr,
     1                 'not implemented for DIS, only option 1!'
                  call pwhg_exit(1)
               endif
            endif
         else
...
c     #        - log(kt2max/pt2)) + xlr
         ...
         if(kn_masses(em).ne.0) then
            if(flg_dis_fsr) then
               write(*,*)'massive FSR emitter not implemented in DIS'
               call pwhg_exit(1)                  
            endif
...
c before a normal exit; not used here
      ...
      if (rad_iupperisr == 1) then
         pt2solve_dis = xlr + cunorm*Pi/b0*(Log(pt2/sborn) + Log(4d0*sborn/xlam2c)*Log(Log(sborn/xlam2c)/Log(pt2/xlam2c)))
      else
         pt2solve_dis = xlr + cunorm*Pi/b0*(Log((4d0*pt2)/sborn) + Log(4d0*sborn/xlam2c)*Log(Log(sborn/(4d0*xlam2c))/Log(pt2/xlam2c)))
      endif
      end
...
c appendix D in ZZ paper.
      ...
      real * 8 t
      real * 8 x,y,x1b,x2b
      real * 8 xlr,q2,xlam2c,kt2max,unorm,sborn, shadron
      integer nlc, nc5s
      common/cpt2solve/xlr,q2,kt2max,xlam2c,unorm,sborn,nlc
     ...
     #   value,err,tmp1,tmp2,tmp,rvalue,born,sig
      ...
      common/cdfxmin/xmin
      real * 8 tmax
      real * 8 yDIS, endpoint, lambda, ximax, radicand, phi3, phi5, branchfactor, c5s(4), c5max, r, valuem, valuep
      common/ctmax/tmax
      real * 8 random,pt2solve,dfxmin,pwhg_alphas0,pwhg_upperb_rad,invmsq, pt2solve_dis, EllipticPiDiff, elliptic_fm, elliptic_inc_fm
      external random,pt2solve,dfxmin,pwhg_alphas0,pwhg_upperb_rad,invmsq, pt2solve_dis
      logical nohit, twobranches
      unorm=rad_norms(rad_kinreg,rad_ubornsubp)
      sborn=kn_sborn
      x1b=kn_xb1
      x2b=kn_xb2
      shadron=sborn/(x1b*x2b)
c See Notes/kt2max.pdf
      if (flg_dis_isr) then
         ! Overestemated maximum
         ! kt2max = sborn/(4*x2b)

         yDIS = -invmsq(kn_cmpborn(:,1) - kn_cmpborn(:,3))/(kn_sborn)
         ...
         kt2max = (shadron-sborn)**2/4/shadron
      endif
...
c eps: required accuracy
      if (flg_dis_isr) then
         call dzero(ptsqmin,kt2max,t,err,1d-8,1000000,pt2solve_dis)
      else
...
c (typo: in D.13, log log -> log
      if(flg_dis_isr) then
         r = t/sborn
         if (rad_iupperisr == 1) then
            tmp1 = log(1d0+1/sqrt(r))
            tmp1 = tmp1/(log(4d0/r)/2d0)
         else
            tmp1 = (-2d0*Sqrt(r)*elliptic_inc_fm(Pi/4d0,2d0 + 1d0/(-0.5d0 + dSqrt(r))) + 2d0*dSqrt(r)*elliptic_fm(2d0 + 1d0/(-0.5d0 + dSqrt(r))))/dSqrt(1d0 - 2d0*dSqrt(r)) - EllipticPiDiff(r)/2d0
            tmp1 = tmp1/(log(2d0/sqrt(r))/2d0)
         endif
      else
...
c using       d sqrt(xm-x) /sqrt(xp-xm) (rad_iupper=1) or d sqrt(xm-x) /(xmin sqrt(xp-xm)) (rad_iupper=2) as upper bound using hit and miss
      if (flg_dis_isr) then
         if (rad_iupperisr == 1) then
            chi = random()
            kn_csi = r + (1 + Sqrt(r))**chi*r**(0.5 - chi/2.) - (1 + Sqrt(r))**chi*r**(1 - chi/2.)
            x=1d0-kn_csi
         else
            nohit = .True.
            do while (nohit)
               chi = random()
               kn_csi = chi**2*(1 - 2*Sqrt(r)) + 2*Sqrt(r)
               x=1d0-kn_csi
               if (random().lt.2d0*Sqrt(((-1 + 2*Sqrt(r))*r*kn_csi)/((4*r - kn_csi)*(2*Sqrt(r) + kn_csi)))) nohit = .False.
            enddo
         endif
      else
...
c get y (abs to avoid tiny negative values)
      if (flg_dis_isr) then
         if (rad_iupperisr == 1) then
            y=(-(-1d0 + x)**2 + r*(1d0 + x))/((-1d0 + x)*(-1d0 + r + x))
         else
            y=sqrt(abs((kn_csi**2 - 4*r)/(kn_csi*(kn_csi - 4*r))))
...
c Veto if out of range (x1>1 or x2>1)
      phi5 = -2*pi*random() + Pi/2d0
      kn_y=y
      kn_azi=-phi5 + Pi/2
      if (flg_dis_isr) then
         endpoint = (-2*(-1 + y)*(-1 + yDIS)*Cos(phi3 - phi5)**2*(-1
     $           - 2*y - y**2 - yDIS + y**2*yDIS - 3*yDIS**2 - y**2
     $           *yDIS**2 + (-1 + y**2)*(-1 + yDIS)*yDIS*Cos(2*phi3 - 2
     $           *phi5) +Sqrt((1 + y - 2*y*yDIS)**2 - 4*(-1 + y**2)*(
     $           -1 + yDIS)*yDIS*Cos(phi3 - phi5)**2) + y*Sqrt((1 + y
     $           - 2*y*yDIS)**2 - 4*(-1 + y**2)*(-1 + yDIS)*yDIS
     $           *Cos(phi3 - phi5)**2) +2*yDIS*Sqrt((1 + y - 2*y*yDIS)
     $           **2 - 4*(-1 + y**2)*(-1 + yDIS)*yDIS*Cos(phi3 - phi5)
     $           **2)))/((1 + y)*yDIS*(2 - yDIS - y*yDIS + (-1 + y)*(
     $           -1 + yDIS)*Cos(2*phi3 - 2*phi5) + Sqrt((1 + y - 2*y
     $           *yDIS)**2 - 4*(-1 + y**2)*(-1 + yDIS)*yDIS*Cos(phi3 -
     $           phi5)**2))**2)
         call compcsimaxisr
         if (kn_csi > kn_csimax_minus) goto 1
         twobranches =  (Cos(phi3-phi5) < 0d0 .and. endpoint > x2b .and. kn_csi > kn_csimin)
      endif
      if (.not.flg_dis_isr) then
         if(x < x2b) then
!     The new incoming parton will be p2Born/(1-csi)= p2Born/x
!     So x must be smaller than the orginal x fraction 
            goto 1
         endif
      endif
...
c     if(flg_newrad) then
      ! For DIS kn_emitter is the only valid option
      if (flg_dis_isr) then
         kn_emitter = 2
         if (twobranches) then
            kn_branchfactor = 1d0
            ...
            valuep=abs(sig*kn_jacreal)
            kn_branchfactor = -1d0
            call gen_real_phsp_isr_rad_g
            call sigreal_rad(sig)
            valuem=abs(sig*kn_jacreal)
            value = valuem + valuep
         else
            ! When only one branch is possible, is always the - 1
            kn_branchfactor = -1d0 
            call gen_real_phsp_isr_rad_g
            call sigreal_rad(sig)
...
c        Set flag for ub failures
      ...
      if (twobranches) then
         if (random() < valuep/ value) then
            ! Recalculate the kinematics with the + branch, as the last attempt was with the - branch (which is usually much more likely)
            kn_branchfactor = 1d0
            call gen_real_phsp_isr_rad_g
         endif
      endif
...
c      if(flg_newrad) then
      ...
      if (flg_dis_fsr) then
         ! It can happen that we get a non-physical point with the dis mapping 
         ! i.e. x > 1
...
c      endif
     ...
     1        'upper bound failures in generation of radiation')
      ...
      subroutine handle_radiations(mode,alr,t,csi,y,azi,branchfactor)
...
c           the highest t in the given resonance.
      ...
      character * (*) mode
      integer alr
      real * 8 t,csi,y,azi, branchfactor
     ...
     1         ts(maxemissions), branches(maxemissions)
      logical valid(maxemissions)
      save valid
      save numemissions,alrs,ts,csis,ys,azis, branches
      ...
      elseif(mode.eq.'save') then
         ...
         branches(numemissions) = branchfactor
